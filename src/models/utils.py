...
def getMparamImagesList(path):
    u"""
    任意のpathにある.pngをlistで返す
    :param path: ../Documents/maya/.../MaterialAssignTool/mparam/
    :return: list [.png, .png,,,, .png]
    """
    files = glob.glob("%s/*" % path)
    pnglist = [file.replace("\\", "/") for file in files if os.path.splitext(file)[1] == ".png"]

    return pnglist


def getWalkMparamImagesList(path):
    fileslist = []
    for current, subfolders, subfiles in os.walk(path):
        if subfiles != []:
            for file in subfiles:
                if ".mparam.png" in file:
                    fileslist.append(refreshPath(current, file))
    return fileslist


def getWalkMparamJsonList(path):
    fileslist = []
    for current, subfolders, subfiles in os.walk(path):
        if subfiles != []:
            for file in subfiles:
                if ".mparam.json" in file and ".mparam.json.meta" not in file:
                    fileslist.append(refreshPath(current, file))
    return fileslist


def getWalkMparamJsonMetaList(path):
    fileslist = []
    for current, subfolders, subfiles in os.walk(path):
        if subfiles != []:
            for file in subfiles:
                if ".mparam.json.meta" in file:
                    fileslist.append(refreshPath(current, file))
    return fileslist


def sortMparamJson(path, fullpath=False):
    """
    .mparam.jsonを取得する
    """
    files = os.listdir(path)
    if fullpath is False:
        return [file for file in files if "mparam" in file and ".json" == os.path.splitext(file)[1]]
    else:
        return [refreshPath(path, file) for file in files if "mparam" in file and ".json" == os.path.splitext(file)[1]]


def sortMparamJsonMeta(path, fullpath=False):
    """
    .mparam.meta
    """
    files = os.listdir(path)
    if fullpath is False:
        return [file for file in files if "mparam" in file and ".meta" == os.path.splitext(file)[1]]
    else:
        return [refreshPath(path, file) for file in files if "mparam" in file and ".meta" == os.path.splitext(file)[1]]
